...
const slideEntries = order.map((name) =>
  loadedMDXFiles.find(([file]) => file === name)
);
...
function Slides() {
  ...
  const prev = () =>
    slideIndex.set(
      (activeIndex - 1 + slideEntries.length) % slideEntries.length
    );
      ...
      <div className="absolute top-0 w-full">
        <div className="h-[5px] bg-slate-500 w-full z-100">
          <div
            style={{
              width:
                ((activeIndex / (slideEntries.length - 1)) * 100).toFixed(0) +
                "%",
            }}
